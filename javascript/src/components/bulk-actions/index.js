	...
	render() {
		return <div className="gallery__bulk-actions fieldholder-small">
			{this.props.gallery.bulkActions.options.map((option, i) => {
				return <button className="gallery__bulk-actions_action font-icon-trash ss-ui-button ui-corner-all" key={i} onClick={this.onChangeValue} value={option.value}>{option.label}</button>;
			})}
		</div>;
	}
